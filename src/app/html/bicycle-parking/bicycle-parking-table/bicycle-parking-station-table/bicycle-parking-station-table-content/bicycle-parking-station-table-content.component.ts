...
@Component({
  selector: 'howell-bicycle-parking-station-table-content',
  imports: [CommonModule, PaginatorComponent, ImageDirective],
  templateUrl: './bicycle-parking-station-table-content.component.html',
  styleUrl: './bicycle-parking-station-table-content.component.less',
  providers: [GarbageStationTableBusiness, GarbageStationTableConverter],
})
export class BicycleParkingStationTableContentComponent
  ...
  implements OnInit
{
  ...
  on = {
    ...
    details: (e: Event, item: GarbageStationTableModel) => {
      if (item) {
        this.details.emit(item.GarbageStation);
      }
      if (this.selected === item) {
        e.stopPropagation();
      }
