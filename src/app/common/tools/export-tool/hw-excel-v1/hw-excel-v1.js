...
class HowellExcelV1 {
  ...
  async read (opts, cb) {
    ...
    var t = await this.zip.file(opts.file).async("string");
...
*/
  ...
  */
  ...
  async generate (fileData, writeName, row, col, v2Chart, replaceWorksheetAttr) {
    ...
    let me = this, copyXml = async (xmlFile, r) => {
      var t = await me.readZip.file(xmlFile).async("string")
      var parser = new xml2js.Parser({ explicitArray: false });
      parser.parseString(t, function (err, o) {
        if (err) {
          return new VError(err, "getXML");
    ...
    await this.readZip.loadAsync(fileData);
    await copyXml("xl/sharedStrings.xml");
    await copyXml('xl/worksheets/sheet1.xml', replaceWorksheetAttr);
    this.setTemplateName();
    if (v2Chart)
      await me.writeChartV2(1, row, col);
    else await me.writeChart(1, row, col);
    setTimeout(async () => {
      var result = await me.zip.generateAsync({ type: 'blob' });
...
exports.HowellExcelV1 = HowellExcelV1;
